...
function applyTheme(theme) {
    ...
    toggleDarkBtn.textContent = theme === "dark" ? "☀️ Light Mode" : "🌙 Dark Mode";
...
function showPage(page) {
    if (page === 'mainTask') {
        ...
        showMainTaskBtn.classList.add('bg-pink-300', 'dark:bg-pink-700', 'text-pink-800', 'dark:text-pink-100');
        showMainTaskBtn.classList.remove('bg-white', 'dark:bg-gray-700', 'text-gray-600', 'dark:text-gray-300');
        showAllTasksBtn.classList.remove('bg-pink-300', 'dark:bg-pink-700', 'text-pink-800', 'dark:text-pink-100');
        showAllTasksBtn.classList.add('bg-white', 'dark:bg-gray-700', 'text-gray-600', 'dark:text-gray-300');
    } else if (page === 'allTasks') {
        bookContainer.classList.remove('show-left');
        bookContainer.classList.add('show-right');
        showAllTasksBtn.classList.add('bg-pink-300', 'dark:bg-pink-700', 'text-pink-800', 'dark:text-pink-100');
        showAllTasksBtn.classList.remove('bg-white', 'dark:bg-gray-700', 'text-gray-600', 'dark:text-gray-300');
        showMainTaskBtn.classList.remove('bg-pink-300', 'dark:bg-pink-700', 'text-pink-800', 'dark:text-pink-100');
        showMainTaskBtn.classList.add('bg-white', 'dark:bg-gray-700', 'text-gray-600', 'dark:text-gray-300');
    }
}
    ...
    if (currentMainTask) {
        ...
        if (currentMainTask.subtasks && currentMainTask.subtasks.length > 0) {
            currentMainTask.subtasks.forEach(step => {
                ...
                li.addEventListener('click', () => toggleStepComplete(currentMainTask.id, step.id));
                stepList.appendChild(li);
            });
            progressBarContainer.classList.remove('hidden');
        } else {
            stepList.innerHTML = '<p class="italic text-gray-600 dark:text-gray-400">No steps added for this task.</p>';
        ...
        taskInput.value = '';
        stepList.innerHTML = '<p class="italic text-gray-600 dark:text-gray-400">No main task selected. Add a new task!</p>';
...
async function renderTasksOnRightPanel() {
    ...
    if (tasks.length === 0) {
        taskList.innerHTML = '<p class="italic text-gray-600 dark:text-gray-400">No tasks added yet.</p>';
        return;
    }

    tasks.forEach(task => {
        const taskElement = document.createElement('div');
        taskElement.className = 'task-item p-4 rounded-xl shadow-sm flex items-center justify-between';
        taskElement.innerHTML = `
            <span class="${task.completed ? 'line-through text-gray-500' : ''}">${task.title}</span>
            <div class="flex items-center gap-2">
                <input type="checkbox" data-id="${task.id}" class="form-checkbox h-5 w-5 text-pink-600" ${task.completed ? 'checked' : ''}>
                <button data-id="${task.id}" class="btn-yellow px-3 py-1 rounded-md text-sm">Select</button>
                <button data-id="${task.id}" class="btn-red px-3 py-1 rounded-md text-sm">Delete</button>
            </div>
        `;
        taskList.appendChild(taskElement);

        // Add event listeners for new elements
        taskElement.querySelector('input[type="checkbox"]').addEventListener('change', (e) => toggleTaskComplete(task.id, e.target.checked));
        taskElement.querySelector('.btn-yellow').addEventListener('click', () => selectMainTask(task.id));
        taskElement.querySelector('.btn-red').addEventListener('click', () => deleteTask(task.id));
    ...
    if (title) {
        try {
            const response = await fetch(`${API_BASE_URL}/tasks`, {
                ...
                body: JSON.stringify({ title, completed: false, isMainTask: false, subtasks: [] })
            });
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
async function toggleStepComplete(taskId, stepId) {
    ...
    const stepIndex = currentMainTask.subtasks.findIndex(step => step.id === stepId);
    if (stepIndex > -1) {
        currentMainTask.subtasks[stepIndex].completed = !currentMainTask.subtasks[stepIndex].completed;
        try {
            const response = await fetch(`${API_BASE_URL}/tasks/${taskId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
...
function updateMainTaskProgressBar() {
    if (currentMainTask && currentMainTask.subtasks && currentMainTask.subtasks.length > 0) {
        const completedSteps = currentMainTask.subtasks.filter(step => step.completed).length;
...
function toggleTimerMode() {
    ...
    if (isFocusTime) {
        ...
        // You might want to play a sound for focus time start
    } else {
        timeLeft = 5 * 60; // 5 minutes for break
        // You might want to play a sound for break time start
    }
    updateTimerDisplay();
...
async function renderMusicPlaylist() {
    ...
    if (musicPlaylist.length === 0) {
        musicPlaylistUl.innerHTML = '<p class="text-gray-600 dark:text-gray-400 text-center">Your playlist is empty. Add a YouTube URL!</p>';
    ...
    musicPlaylist.forEach((tune, index) => {
        const li = document.createElement('li');
        li.className = `flex items-center justify-between p-2 rounded-md ${index === currentTuneIndex ? 'bg-purple-200 dark:bg-purple-700' : ''}`;
        li.innerHTML = `
            <span class="tune-title-editable flex-1 cursor-pointer truncate mr-2 ${index === currentTuneIndex ? 'font-bold' : ''}">${tune.title || 'Unknown Title'}</span>
            <div class="flex items-center gap-2">
                <button data-id="${tune.id}" data-index="${index}" class="play-tune-btn btn-purple px-2 py-1 rounded-md text-sm">▶️ Play</button>
                <button data-id="${tune.id}" class="delete-tune-btn btn-red px-2 py-1 rounded-md text-sm">🗑️</button>
            </div>
        `;
        ...
        li.querySelector('.delete-tune-btn').addEventListener('click', () => deleteTune(tune.id));
        ...
        tuneTitleSpan.addEventListener('dblclick', () => {
            ...
            input.className = 'w-full px-1 py-0.5 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-black dark:text-white';
...
function prevTune() {
    ...
    currentTuneIndex = (currentTuneIndex - 1 + musicPlaylist.length) % musicPlaylist.length;
    playTune(currentTuneIndex);
}
...
async function deleteTune(tuneId) {
    if (confirm('Are you sure you want to remove this tune from the playlist?')) {
        try {
            ...
            const deletedIndex = musicPlaylist.findIndex(tune => tune.id === tuneId);
            if (deletedIndex !== -1) {
                ...
                if (deletedIndex === currentTuneIndex) {
                    ...
                    if (musicPlaylist.length > 0) {
                        ...
                        if (player && typeof player.loadVideoById === 'function') { // Ensure player methods exist
                             player.loadVideoById(musicPlaylist[currentTuneIndex].videoId);
